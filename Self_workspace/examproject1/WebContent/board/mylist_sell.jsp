...
<body>
	<%
		...
		int totalRecord = dao.getMyListSize((String)session.getAttribute("id"));
		int totalPage = 0;
		int totalBlock = 0;
		int numPerPage = 10;
		int pagePerBlock = 5;
		...
		ArrayList<BoardDto> list = (ArrayList<BoardDto>)dao.getMyList(beginPerPage, numPerPage, (String)session.getAttribute("id"));
	%>
	...
	<div id="ui_top_banner">
		<div id="wrap">
			...
			<ul id="ui_top_menu">
				<li><a href="mylist_sell.jsp" class="ui_selected">판매목록</a></li>
				<li><a href="">경매목록</a></li>
				<li><a href="mylist_buy.jsp">구입목록</a></li>
				<li><a href="">관심목록</a></li>
				<div class="clear"></div>
			</ul>
			<div class="clear"></div>
	...
	<div id="wrap" class="wrap_min">
		<article>
			...
			<div id="page">
				...
				if(nowBlock > 0) {
					%>
					<a href="javascript:send('mylist_sell.jsp', <%=numPerPage %>, <%=(nowBlock - 1) * pagePerBlock %>, <%=nowBlock - 1 %>);">◀</a>
					<%
				}
				for(int i = 0; i < pagePerBlock; i++) {
					if(nowBlock * pagePerBlock + i == totalPage) break;
					%><a href="javascript:send('mylist_sell.jsp', <%=numPerPage %>, <%=nowBlock * pagePerBlock + i %>, <%=nowBlock %>);"><%=nowBlock * pagePerBlock + i + 1 %></a><%
				}
				if(nowBlock + 1 < totalBlock) {
					%>
						<a href="javascript:javascript:send('mylist_sell.jsp', <%=numPerPage %>, <%=(nowBlock + 1) * pagePerBlock %>, <%=nowBlock + 1 %>);">▶</a>
					<%
				}
				%>
			</div>
