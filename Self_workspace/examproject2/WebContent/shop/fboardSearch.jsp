...
<%
	...
	// 전체 글 개수 가져오기
	int count = fdao.getCount(search);
	...
	if(count>0){
		list = fdao.getBoardList(startRow,pageSize,search);
	}
...
<body>
	...
	%>
		<tr style="cursor: default;">
			...
			%><a href="fcontent.jsp?pageNum=<%=pageNum%>&num=<%=bean.getNum()%>"> 
				<%=bean.getSubject() %>
			  </a>
			</td>
			<td><%=bean.getName() %></td>
	...
	<div id="table_search" style="float:inherit;">
		<form action="fboardSearch.jsp">
			...
			<span id="table_search" style="float: none;">
				<input class="board_btn01" type="button" value="파일글쓰기" onclick="location.href='write.jsp'">
			</span>
			<%
					}
			%> 
	...
	<%
		...
		%>		<a href="fboard.jsp?pageNum=<%=startPage-pageBlock %>&search=<%=search%>">[이전]</a>
		<%	}
			for(int i=startPage;i<=endPage;i++){
		%>		<a href="fboardSearch.jsp?search=<%=search%>">[<%=i %>]</a>
		<%	}
			if(endPage<pageCount){
		%>		<a href="fboard.jsp?pageNum=<%=startPage+pageBlock %>&search=<%=search%>">[다음]</a>		
		<% }
		}
	%>
	</article>
