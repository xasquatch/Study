		...
		if (action != null){
			...
			if(action.equals("/listMembers.do")) {
				...
				if (check == 0) {
					...
					nextPage = "/test03/TemperalyNotice.jsp";
				...
				MemberValueObject vo = dao.findmember(id);
				...
				MemberValueObject vo = new MemberValueObject(id, pwd, name, email);
				
				int check = dao.modmember(vo);

				if (check == 0) {
					System.out.println("안된거야");//임시
					nextPage = "/member/listMembers.do";
					request.setAttribute("notice", "회원정보 수정에 실패하였습니다. 다시 시도해주세요.");
					
				} else {
					System.out.println("완료");//임시
					nextPage = "/member/listMembers.do";
					request.setAttribute("notice", "회원정보가 정상적으로 수정되었습니다.");
