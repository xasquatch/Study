...
<body>
			...
			<c:otherwise>
				<c:forEach items="${list}" var="memberVO">
					<tr align="center">
						...
						<td><a href='${contextPath}/member/modmemberform.do?id=${memberVO.id }'>MOD</a></td>
						<td><a href='${contextPath}/member/delmember.do?id=${memberVO.id }'>DEL</a></td>
					</tr>
				</c:forEach>
			</c:otherwise>
		</c:choose>
	...
	<c:choose>
		<c:when test="${empty session.id}">
			<a href="${contextPath}/member/memberForm.do" class="cls2">
			<p>JOIN US NOW!</p></a>
		</c:when>
	</c:choose>
