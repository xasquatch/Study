...
<div id="wrap">
		...
		<div id="section">
			...
			<form action="search.do" method="post" name="search_form">
				<label class="total_count">전체 글 수 : ${count}</label>
				<input type="button" value="새 글 쓰기" class="write_btn">
				<table>
					<tr>
					...
					<c:forEach var="list" items="${list}">
    					 <tr>
        					...
        					<td onclick="location.href='view.do?num=${list.num}&pageNum=${pageNum}'"><label>${list.title}</label></td>
				...
				<p>
					<c:if test="${pageNum ne 1}">
						<p  class="page"><a href="board.do?pageNum=${pageNum-1}"><label> 이전 </label></a></p>
					</c:if>
    					<p  class="page"><a href="board.do?pageNum=${pageNum}"><label class="page">${pageNum}</label></a></p>
					<c:if test="${pageNum lt count/pageSize}">
						<p  class="page"><a href="board.do?pageNum=${pageNum+1}"><label class="page"> 다음 </label></a></p>
					</c:if>
				</p>
				<select name="select">
