...
<!-- 	자바코드로 new MemberBean()객체 생성X → useBean액션태그로 MemberBean객체 생성
 ...
 -->
	<jsp:useBean id="m" class="sec01.ex01.MemberBean" scope="page"></jsp:useBean>
			<%--
	 		회원가입창(memberform.html)에서 전달된 input태그의 name속성과 
	 		<jsp:setProperty>액션태그의 param속성의 값이 같으면 
	 		MemberBean객체의 변수에 자동으로 저장됨
	 		조건1 : memberBean클래스의 각 변수이름과 MemberForm.html페이지의 <input name="값">이 동일해야함
	 		조건2 : memberForm.html페이지의 input태그의 name속성값과 param속성값이 동일해야함
	 		 --%>
...
<%
// 		//useBean액션태그로 부터 생성된 MemberBean객체에 memberForm.html로부터 전송된 회원정보를 저장
// 		m.setId(id);
// 		m.setPwd(pwd);
// 		m.setName(name);
// 		m.setEmail(email);
